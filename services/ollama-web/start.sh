...
start_main(){
    ...
    # Hack, use volumes section in docker compose file to make dirs
    mkdir -p /data/service/docker/open-webui-postgres
    mkdir -p /data/service/docker/open-webui
    mkdir -p /data/service/docker/ollama

    ensure_env "$SERVICE_DPATH"
...
ensure_env() {
  local dpath="$1"
  local envfile="$dpath/.env"

  # Create .env if missing
  if [[ ! -f "$envfile" ]]; then
    echo "No .env found; creating $envfile"
    local pw user db port
    pw="$(make_secret)"
    user="openwebui"
    db="openwebui"
    port="${OPENWEBUI_PORT:-14771}"

    cat >"$envfile" <<EOF
# --- Generated by start.sh on $(date -Iseconds) ---
# EDIT SAFELY: these are used by docker-compose
POSTGRES_DB=${db}
POSTGRES_USER=${user}
POSTGRES_PASSWORD=${pw}
OPENWEBUI_PORT=${port}
# Example DATABASE_URL that open-webui uses
DATABASE_URL=postgresql://${user}:\${POSTGRES_PASSWORD}@postgres:5432/${db}
EOF
    chmod 600 "$envfile"
    echo "Created $envfile (600)."
  fi

  # Export values so the script can use them (e.g., OPENWEBUI_PORT)
  set -a
