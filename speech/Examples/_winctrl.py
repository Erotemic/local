...
config.settings.grid = Item(
    10, doc="The number of grid divisions a monitor is divided up into when placing windows.")
...
class StretchRule(CompoundRule):
    ...
    def _process_recognition(self, node, extras):
        ...
        if child:
            horizontals.append(monitor.x1 + child.value() * monitor.dx)
        x1, x2 = min(horizontals), max(horizontals)
        ...
        if child:
            verticals.append(monitor.y1 + child.value() * monitor.dy)
        y1, y2 = min(verticals), max(verticals)
