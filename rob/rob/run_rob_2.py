...
class ROB_Directories:
    ...
    def build_paths(d):
        ...
        d.USERPROFILE = d.ROOT + \
            home_map[os_type] + slash_fix('/' + d.USERNAME)
...
class ROB_Files:
    def __init__(f, d):
        ...
        f.winedt_exe = d.INSTALL32 + \
            slash_fix('/WinEdt Team/WinEdt 7/WinEdt.exe')
        f.rap_ee_exe = d.PORT_APPS + \
            slash_fix('/RapidEnvirornmentEditor/RapidEE.exe')
        f.qiqqa_exe = d.INSTALL32 + slash_fix('/Qiqqa/Qiqqa.exe')

        f.spotify_exe = d.INSTALL32 + slash_fix('/spotify.exe')
        if comp_name == "Termina":
            f.spotify_exe = r'C:\Users\jon.crall\AppData\Roaming\Spotify\spotify.exe'
        if comp_name == "Ooo":
            f.spotify_exe = r'C:\Program Files (x86)\Spotify\spotify.exe'

        f.vim_exe = d.VIM_BIN + slash_fix('/gvim.exe')
        chrome_root = d.INSTALL32
        if comp_name == 'Ooo':
            chrome_root = d.USERPROFILE + '/AppData/Local'
        f.chrome_exe = chrome_root + \
            slash_fix('/Google/Chrome/Application/chrome.exe')
        f.matlab_exe = d.ROOT + slash_fix('/MATLAB/R2012a/bin/matlab.exe')

        f.crallj_ahk = d.AHK_SCRIPTS + slash_fix('/crallj.ahk')
        f.create_shortcut_vbs = d.WIN_SCRIPTS + \
            slash_fix('/Helpers/CreateShortcutHelper.vbs')
