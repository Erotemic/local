...
my $url = 'http://mxr.mozilla.org/mozilla-central/source/security/nss/lib/ckfw/builtins/certdata.txt?raw=1';
...
while (<TXT>) {
  ...
  if (/^CKA_LABEL\s+[A-Z0-9]+\s+\"(.*)\"/) {
    $caname = $1;
    if ($caname =~ /Explicitly Distrust/i) {
        print "not trusting $caname\n";
    	$badcert = 1;
    }
  }
