...
public:
    ...
    void begin(){
        ...
        this->prev_time = this->now_wall();
        printf("\33[2K\r%s", lbl.c_str());
        fflush(stdout);
    }
    ...
    void mark(int i)
    {
        ...
        printf("\33[2K\r%s %d/%d rate=%.2fHz, etr=%ds, total=%ds", 
                lbl.c_str(), i, nTotal, this->iter_per_sec, (int) this->est_sec_remain, (int) this->total_sec);
    ...
    void mark(int i, const char * format, ...){
        ...
        ensure_buffer(nbytes_need);
        vsnprintf(this->extra_buffer, nbytes_have, format, args);
        va_end(args);
        this->update_measures_wall(i);
        //const char* CLEARLINE_EL2 = "\33[2K";
        printf("\33[2K\r%s %d/%d rate=%.2fHz, etr=%ds, total=%ds %s", 
                lbl.c_str(), i, nTotal, this->iter_per_sec, 
                (int) this->est_sec_remain, (int) this->total_sec, this->extra_buffer);
        this->flush_if_requested();
    }
...
protected:
    ...
    void update_measures_wall(int i)
    {
        ...
        this->sec_since_last = (this->current_time - this->prev_time);
        this->total_sec = (this->current_time - this->start_time );
        this->iter_per_sec = (double) (i) / this->total_sec;
        this->est_sec_remain = (double) (nTotal - i) / this->iter_per_sec;
    }

    void flush_if_requested()
    {
        if (this->sec_since_last > flush_freq_sec)
        {
            fflush(stdout);
            this->prev_time = this->now_wall();
        }
...
int main(int argc, char** argv)
{
    ...
    clang progiter.cpp  -lstdc++
     */
    int N = 1000;
    ProgIter prog = ProgIter(N, "[TestProgIter]");
    prog.begin();
