    #pytest --cov=vital --cov-report=term --tb=no --disable-warnings --verbose --pyargs vital 
    #coverage run --source vital/tests/*.py -m py.test
    #coverage run --source sprokit -m py.test
    #coverage report

    #mkdir -p covd
    #python -m trace --count --coverdir covd --ignore-dir="\$prefix" --ignore-dir="\$exec_prefix" sprokit/tests/test-pymodules.py 

    #pytest --cov=sprokit -q --tb=no --disable-warnings

    #pytest --cov=sprokit --cov-report=term  --tb=no --disable-warnings --pyargs sprokit

~/code/VIAME/packages/kwiver/vital/bindings/
~/code/VIAME/packages/kwiver/vital/bindings/c_introspect.py

~/code/VIAME/packages/kwiver/vital/bindings/c/types/detected_object.h
~/code/VIAME/packages/kwiver/vital/bindings/c/types/detected_object.cxx

~/code/VIAME/packages/kwiver/vital/types/detected_object.cxx
~/code/VIAME/packages/kwiver/vital/bindings/python/vital/types/detected_object.py
~/code/VIAME/packages/kwiver/vital/bindings/python/vital/types/c_introspect.py



# CMAKE UTILITIES
# ---------------
~/code/VIAME/packages/kwiver/CMake/kwiver-test-setup.cmake
~/code/VIAME/packages/kwiver/CMake/kwiver-utils.cmake
~/code/VIAME/packages/kwiver/CMake/tools/discover_python_tests.py
# kwiver utilities
~/code/VIAME/packages/kwiver/CMake/utils/kwiver-utils-configuration.cmake
~/code/VIAME/packages/kwiver/CMake/utils/kwiver-utils-python-tests.cmake
~/code/VIAME/packages/kwiver/CMake/utils/kwiver-utils-python.cmake
~/code/VIAME/packages/kwiver/CMake/utils/kwiver-utils-tests.cmake

# sprokit utilities
~/code/VIAME/packages/kwiver/sprokit/cmake/conf/sprokit-macro-targets.cmake
~/code/VIAME/packages/kwiver/sprokit/cmake/conf/sprokit-macro-tests.cmake
~/code/VIAME/packages/kwiver/sprokit/cmake/conf/sprokit-macro-configure.cmake
~/code/VIAME/packages/kwiver/sprokit/cmake/conf/sprokit-macro-python.cmake
~/code/VIAME/packages/kwiver/sprokit/cmake/support/test.cmake
~/code/VIAME/packages/kwiver/sprokit/tests/bindings/python/CMakeLists.txt  # Defines helpers


# ------------
# SOURCE FILES

# KWIVER 
~/code/VIAME/packages/kwiver/CMakeLists.txt
# vital python tests
~/code/VIAME/packages/kwiver/vital/bindings/python/vital/tests/CMakeLists.txt
~/code/VIAME/packages/kwiver/vital/bindings/python/run_tests.sh

# SPROKIT 
~/code/VIAME/packages/kwiver/sprokit/processes/bindings/python/kwiver/util/vital_type_converters.py


# sprokit source
~/code/VIAME/packages/kwiver/sprokit/processes/bindings/python/CMakeLists.txt
~/code/VIAME/packages/kwiver/sprokit/src/bindings/python/CMakeLists.txt

# sprokit python tests
~/code/VIAME/packages/kwiver/sprokit/tests/bindings/python/CMakeLists.txt  # Defines helpers
~/code/VIAME/packages/kwiver/sprokit/tests/bindings/python/sprokit/pipeline/CMakeLists.txt
~/code/VIAME/packages/kwiver/sprokit/tests/bindings/python/sprokit/pipeline_util/CMakeLists.txt
~/code/VIAME/packages/kwiver/sprokit/tests/bindings/python/modules/CMakeLists.txt

# misc sprokit python
~/code/VIAME/packages/kwiver/sprokit/src/sprokit/python/any_conversion/CMakeLists.txt


# -----------
# BUILD FILES

# SPROKIT TEST/BIN 
~/code/VIAME/build/build/src/kwiver-build/sprokit/tests/bin

# PYTHON-PATH BUILD INSTALL
~/code/VIAME/build/build/src/kwiver-build/lib/python2.7/site-packages/sprokit/tests

~/code/VIAME/build/install/setup_viame.sh
